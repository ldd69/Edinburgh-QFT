...
\newcommand{\ISProp}[1]{\frac{i}{#1^2-m^2+i\epsilon}}
\newcommand{\HSProp}[1]{\frac{1}{#1^2-M^2+i\epsilon}}
\newcommand{\IHSProp}[1]{\frac{i}{#1^2-M^2+i\epsilon}}
\newcommand{\ESProp}[2]{\frac{1}{\left(#1^2+m^2\right)^#2}}
\newcommand{\EHSProp}[2]{\frac{1}{\left(#1^2+M^2\right)^#2}}
...
\begin{equation}
  ...
  (2\pi)^D \delta\left(p+p'\right) i\tilde{\Delta}_F(p)\, ,
\end{equation}
...
remember that it is there, and focus on the perturbative expansion of
the {\em full} propagator 
\begin{equation}
  \label{eq:PropMomPert}
  i\tilde{\Delta}_F(p) = \sum_k g^k i\tilde{\Delta}^{(k)}_F(p)\, .
\end{equation}
The term {\em full} here refers to the fact that we want to 
take into account the contributions from the interaction 
termi in the potential. Ths suffix $F$ is meant to remind us
that we are considering the full propagator. 
From our previous computations, the 1PI one-loop contribution
to the propagator is 
\begin{align}
   g^2 i \tDelta_F^{(2)}(p)
  &= \frac{1}{2} (ig)^2\, \ISProp{p} \left(\int \frac{d^D\ell}{(2\pi)^D}\,
    \ISProp{\ell} \ISProp{(\ell-p)}\right) \times \nonumber \\
  &\quad \times \ISProp{p}\, .,
\end{align}
...
\begin{equation}
  \label{eq:OneLoopPi}
  i\tDelta(p) \left(-i\Pi(p^2)\right) i\tDelta(p)\, ,
\end{equation}
where
\begin{equation}
  \label{eq:PiFun}
...
\begin{equation}
  ...
  I_D = \int \frac{d^Dq_E}{(2\pi)^D}\, \EHSProp{q_E}{2}\, ,
\end{equation}
which is easily computed in spherical coordinates:
\begin{align}
  I_D &= \int \frac{d\Omega_D}{(2\pi)^D}\,
        \frac12 \int_0^\infty dq_E^2\, (q_E^2)^{D/2-1} \EHSProp{q_E}{2}
  \\
...
\begin{align}
  ...
  \label{eq:ExplicitDivs}
           &= \frac{\alpha}{2} \left[
             ...
             p^2-m^2\right)
             - \int_0^1dx\, M^2(x,p^2) \log\left(\frac{\mu^2}{M^2(x,p^2)}\right)
             \right]
             +O(\epsilon) \, ,
\end{align}
where
